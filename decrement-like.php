...
if(mysqli_num_rows($rslt1)>0)
{
		while($row1=mysqli_fetch_assoc($rslt1))
		{
			...
			if($opinion=="1")
			{
				$upvote=$upvote-1;
				$downvote=$downvote+1;
				$qry3="UPDATE sandesh_likes.$question SET opinion='0' WHERE user_id='$user_id'";
			...
			else if($opinion=="0")
			{
				...
				$qry7="UPDATE sandesh.news SET downvote='$downvote' WHERE id='$question'";
...
{
	$downvote=$downvote+1;
	$qry2="INSERT INTO sandesh_likes.$question (user_id,opinion) VALUES ('$user_id','0')";
	$rslt2=mysqli_query($con1,$qry2);
	$qry5="UPDATE sandesh.news SET downvote='$downvote' WHERE id='$question'";
